...
# Configuration
C_CODE_DIR = "../c_code"
RESULTS_ROOT_DIR = "../benchmark_runs_v2" # Separate results dir for V2
...
def compile_simulation():
    ...
    logger.info("Compiling C simulation code (V2)...")
    try:
        # Compile everything to ensure V2 targets are built
...
def run_simulation(exe, rows, threads, steps):
    ...
    try:
        ...
        # Parse "Wall=X.XXXs" from stderr (V2 prints "Done V2. Wall=...")
        # Regex handles both standard "Done." and "Done V2." formats just in case
...
def main():
    ...
    parser.add_argument("--size-end", type=int, default=2000, help="End Grid Size (inclusive)")
    ...
    parser.add_argument("--iso-start", type=float, default=0.50, help="Start Target Efficiency")
    parser.add_argument("--iso-end", type=float, default=0.99, help="End Target Efficiency")
    parser.add_argument("--iso-step", type=float, default=0.01, help="Target Efficiency Step")
    ...
    run_dir = os.path.join(RESULTS_ROOT_DIR, f"bench_v2_{timestamp}")
    ...
    # Use V2 Executables
    exe_serial = os.path.join(C_CODE_DIR, "sw2d_serial") # Use baseline serial for comparison
    exe_omp = os.path.join(C_CODE_DIR, "sw2d_omp_v2")
    exe_pthread = os.path.join(C_CODE_DIR, "sw2d_pthread_v2")
    ...
    logger.info("Starting V2 Benchmark Suite...")
    ...
    for N in problem_sizes:
        ...
        # 4b. Sweep OpenMP V2
        logger.info(f"  Sweeping OpenMP V2 (N={N})...")
        for T in thread_counts:
            t = run_simulation(exe_omp, N, T, args.steps)
            if t is None: t = 0.0
            results_omp[T][N] = t
            logger.info(f"    [OMP V2] Threads={T}: {t:.4f}s")

        # 4c. Sweep Pthreads V2
        logger.info(f"  Sweeping Pthreads V2 (N={N})...")
        for T in thread_counts:
            t = run_simulation(exe_pthread, N, T, args.steps)
            if t is None: t = 0.0
            results_pthread[T][N] = t
            logger.info(f"    [Pthread V2] Threads={T}: {t:.4f}s")
        ...
        for N in problem_sizes:
            ...
            for T in thread_counts:
                ...
                if t_par > 0:
                    ...
                    speedup = 0.0
                    efficiency = 0.0
                writer.writerow(["OpenMP_V2", N, T, f"{t_par:.6f}", f"{speedup:.6f}", f"{efficiency:.6f}"])
        ...
        for N in problem_sizes:
            ...
            for T in thread_counts:
                ...
                if t_par > 0:
                    ...
                    speedup = 0.0
                    efficiency = 0.0
                writer.writerow(["Pthreads_V2", N, T, f"{t_par:.6f}", f"{speedup:.6f}", f"{efficiency:.6f}"])

    logger.info(f"CSV Export Complete: {csv_file}")

    # --- PLOTTING LOGIC (Identical to V1 but for V2 data) ---
    ...
    plot_heatmap(eff_matrix_omp, problem_sizes, thread_counts, 
                 "OpenMP V2 Efficiency Heatmap", os.path.join(run_dir, "heatmap_efficiency_omp.png"))
    plot_heatmap(eff_matrix_pthread, problem_sizes, thread_counts, 
                 "Pthreads V2 Efficiency Heatmap", os.path.join(run_dir, "heatmap_efficiency_pthread.png"))
    ...
    for i, N in enumerate(problem_sizes):
        ...
        plt.plot(thread_counts, y_omp, marker='o', linestyle='-', color=c, label=f"OMP V2 (N={N})")
        plt.plot(thread_counts, y_pth, marker='x', linestyle='--', color=c, label=f"Pthread V2 (N={N})")
    plt.xlabel("Number of Threads"); plt.ylabel("Time (s)")
    plt.title("Master Comparison: Time vs Threads (OMP V2 vs Pthreads V2)")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    ...
    for i, N in enumerate(problem_sizes):
        ...
        plt.plot(thread_counts, s_omp, marker='o', linestyle='-', color=c, label=f"OMP V2 (N={N})")
        plt.plot(thread_counts, s_pth, marker='x', linestyle='--', color=c, label=f"Pthread V2 (N={N})")
    plt.xlabel("Number of Threads"); plt.ylabel("Speedup")
    plt.title("Master Comparison: Speedup vs Threads (OMP V2 vs Pthreads V2)")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.tight_layout()
    ...
    for i, N in enumerate(problem_sizes):
        ...
        plt.plot(thread_counts, e_omp, marker='o', linestyle='-', color=c, label=f"OMP V2 (N={N})")
        plt.plot(thread_counts, e_pth, marker='x', linestyle='--', color=c, label=f"Pthread V2 (N={N})")
    plt.xlabel("Number of Threads"); plt.ylabel("Efficiency")
    plt.title("Master Comparison: Efficiency vs Threads (OMP V2 vs Pthreads V2)")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    ...
    plt.xlabel("Number of Threads"); plt.ylabel("Time (s)")
    plt.title("Pthreads V2 Master Time Scaling")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.tight_layout()
    ...
    plt.xlabel("Number of Threads"); plt.ylabel("Speedup")
    plt.title("OpenMP V2 Master Speedup Scaling")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.tight_layout()
    ...
    plt.xlabel("Number of Threads"); plt.ylabel("Speedup")
    plt.title("Pthreads V2 Master Speedup Scaling")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.tight_layout()
    ...
    plt.xlabel("Number of Threads"); plt.ylabel("Efficiency")
    plt.title("OpenMP V2 Master Efficiency Scaling")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.ylim(0, 1.2); plt.tight_layout()
    ...
    plt.xlabel("Number of Threads"); plt.ylabel("Efficiency")
    plt.title("Pthreads V2 Master Efficiency Scaling")
    ax.xaxis.set_major_locator(MaxNLocator(integer=True))
    plt.legend(bbox_to_anchor=(1.05, 1), loc='upper left'); plt.grid(True); plt.ylim(0, 1.2); plt.tight_layout()
    ...
    for N in problem_sizes:
        ...
        plot_line_metric(thread_counts, {"OpenMP V2": times_omp}, "Threads", "Time (s)", 
                         f"OpenMP V2 Execution Time (N={N})", os.path.join(size_dir, "individual_time_omp.png"))
        plot_line_metric(thread_counts, {"Pthreads V2": times_pthread}, "Threads", "Time (s)", 
                         f"Pthreads V2 Execution Time (N={N})", os.path.join(size_dir, "individual_time_pthread.png"))

        plot_line_metric(thread_counts, {"OpenMP V2": speedup_omp}, "Threads", "Speedup", 
                         f"OpenMP V2 Speedup (N={N})", os.path.join(size_dir, "individual_speedup_omp.png"), 
                         ideal_series=thread_counts)
        plot_line_metric(thread_counts, {"Pthreads V2": speedup_pthread}, "Threads", "Speedup", 
                         f"Pthreads V2 Speedup (N={N})", os.path.join(size_dir, "individual_speedup_pthread.png"), 
                         ideal_series=thread_counts)

        plot_line_metric(thread_counts, {"OpenMP V2": eff_omp_line}, "Threads", "Efficiency", 
                         f"OpenMP V2 Efficiency (N={N})", os.path.join(size_dir, "individual_efficiency_omp.png"),
                         ideal_series=[1.0]*len(thread_counts))
        plot_line_metric(thread_counts, {"Pthreads V2": eff_pthread_line}, "Threads", "Efficiency", 
                         f"Pthreads V2 Efficiency (N={N})", os.path.join(size_dir, "individual_efficiency_pthread.png"),
                         ideal_series=[1.0]*len(thread_counts))

        plot_line_metric(thread_counts, {"OpenMP V2": times_omp, "Pthreads V2": times_pthread}, 
                         "Threads", "Time (s)", f"Execution Time Comparison (N={N})", 
                         os.path.join(size_dir, "comparison_time.png"))

        plot_line_metric(thread_counts, {"OpenMP V2": speedup_omp, "Pthreads V2": speedup_pthread}, 
                         "Threads", "Speedup", f"Speedup Comparison (N={N})", 
                         os.path.join(size_dir, "comparison_speedup.png"), 
                         ideal_series=thread_counts)

        plot_line_metric(thread_counts, {"OpenMP V2": eff_omp_line, "Pthreads V2": eff_pthread_line}, 
    ...
    plot_line_metric(thread_counts, {f"E={t:.2f}": val for t, val in iso_data_omp.items()}, 
                     ...
                     "OpenMP V2 Iso-Efficiency Function", 
    ...
    plot_line_metric(thread_counts, {f"E={t:.2f}": val for t, val in iso_data_pthread.items()}, 
                     "Threads", "Required Grid Size (N)", 
                     "Pthreads V2 Iso-Efficiency Function", 
    ...
    for i, target in enumerate(subset_targets):
        ...
        if target in iso_data_omp:
            plt.plot(thread_counts, iso_data_omp[target], 'o-', color=c, label=f"OMP V2 (E={target:.2f})")
        if target in iso_data_pthread:
            plt.plot(thread_counts, iso_data_pthread[target], 'x--', color=c, label=f"Pthread V2 (E={target:.2f})")
    ...
    for target in iso_targets:
        ...
        if target in iso_data_omp:
            plt.plot(thread_counts, iso_data_omp[target], 'o-', label=f"OpenMP V2")
            has_data = True
            
        if target in iso_data_pthread:
            plt.plot(thread_counts, iso_data_pthread[target], 'x--', label=f"Pthreads V2")
            has_data = True
